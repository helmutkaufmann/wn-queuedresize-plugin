...
The primary function is to **queue resize jobs for all images within a directory** when the code is executed.
...
| **`$opts`** | `array` | No | An array of options (`mode`, `quality`, etc.) passed to the job. |
...
function onStart()
{
    ...
    $jobsQueued = queuedresize_path(
        'user-uploads/latest-image.jpg', // Path is a file, but resolves to the 'user-uploads' directory
        1024,
        null, 
